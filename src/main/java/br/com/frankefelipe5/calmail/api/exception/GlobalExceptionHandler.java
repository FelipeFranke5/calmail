...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleRequest(RequestException e) {
        return ResponseEntity.badRequest().body(new ErrorResponse(ErrorCode.REQUEST_EXCEPTION.value(), e.getMessage()));
    }
    ...
    public ResponseEntity<ErrorResponse> handleUnsupportedMethod(HttpRequestMethodNotSupportedException e) {
        ...
        return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
                .body(new ErrorResponse(ErrorCode.HTTP_REQUEST_METHOD_NOT_SUPPORTED_EXCEPTION.value(), error));
    }
    ...
    public ResponseEntity<ErrorResponse> handleInvalidArgs(MethodArgumentNotValidException e) {
        ...
        Map<String, Object> detail = new HashMap<>();
        e.getFieldErrors().stream().forEach(errorField -> {
    ...
    public ResponseEntity<ErrorResponse> handleAuthResponseException() {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED.value())
                .body(new ErrorResponse(ErrorCode.AUTH_RESPONSE_EXCEPTION.value(), "please authenticate to proceed"));
    }
    ...
    public ResponseEntity<ErrorResponse> handleHttpClientErrorExternalCall(HttpClientErrorException e) {
    ...
    public ResponseEntity<ErrorResponse> handleHttpClientServerExternalCall(HttpServerErrorException e) {
        HashMap<String, Object> detail = new HashMap<>();
    ...
    public ResponseEntity<ErrorResponse> handleAIResponseAcessNotGrantedException(
            AIResponseAcessNotGrantedException e) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN.value())
                .body(new ErrorResponse(ErrorCode.ACESS_NOT_GRANTED_EXCEPTION.value(), e.getMessage()));
    }

    // Helper functions below

    private ResponseEntity<ErrorResponse> errorMessage(boolean returnIs401, boolean returnIs400, String jsonPart)
    ...
    private ResponseEntity<ErrorResponse> parseErrorMessage(JsonProcessingException notAJson) {
        ...
        return ResponseEntity.internalServerError()
                .body(new ErrorResponse(ErrorCode.PARSING_JSON.value(), "internal server error - parsing error"));
    }
